...
# Healthcheck using Node.js (wget/curl are removed for security)
HEALTHCHECK --interval=30s --timeout=3s --start-period=10s --retries=3 \
  CMD node -e "require('http').get('http://localhost:3000/',r=>process.exit(r.statusCode!==200?1:0)).on('error',()=>process.exit(1))" || exit 1
